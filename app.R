...
ui <- fluidPage(
  ...
  div(class = "form-container",
      ...
      div(class = "section-card",
          h3("📍 Géolocalisation"),
          actionButton("btn_gps", "📡 Obtenir ma position", class = "btn-primary btn-block"),
          br(),
          div(class = "gps-info", uiOutput("gps_display")),
      ...
      div(class = "section-card",
          ...
          div(class = "input-group-custom",
              ...
              numericInput("nb_personnes", label = NULL, value = NULL, min = 1, max = 500, step = 1)
          ),
      ...
      div(class = "action-buttons",
          actionButton("btn_submit", "✅ Enregistrer l'observation", class = "btn-success btn-lg btn-block"),
          br(),
          actionButton("btn_reset", "🔄 Nouveau formulaire", class = "btn-warning btn-block"),
          br(),
  ...
  div(class = "section-card",
      ...
      downloadButton("btn_export", "⬇️ Exporter CSV", class = "btn-info"),
      br(), br(),
...
server <- function(input, output, session) {
  ...
  output$gps_display <- renderUI({
    ...
    } else if (!is.null(input$gps_status) && input$gps_status == "error") {
      div(class = "gps-error", p("❌ Impossible d'obtenir la position"))
    } else {
      div(class = "gps-waiting", p("⏳ En attente de localisation..."))
    }
  })
  
  output$mini_map <- renderLeaflet({
    leaflet() %>% addTiles() %>% setView(lng = 2.3522, lat = 46.8566, zoom = 5)
  })
  ...
  observeEvent(rv$lat, {
    ...
    leafletProxy("mini_map") %>%
      ...
      addMarkers(lng = rv$lon, lat = rv$lat,
                 popup = paste("📍 Position actuelle<br>Précision:", round(rv$accuracy, 0), "m")) %>%
  ...
  observeEvent(input$btn_submit, {
    ...
    if (length(errors) > 0) {
      output$submit_message <- renderUI({
        div(class = "alert alert-danger", lapply(errors, function(e) p(e)))
      })
      return()
    }
    ...
    nouvelle_obs <- data.frame(
      ...
      commentaires        = ifelse(!is.null(input$commentaires) && input$commentaires != "",
